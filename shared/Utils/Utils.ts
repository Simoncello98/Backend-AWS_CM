...
export class Utils {
    ...
    public validateRequestObject(event: APIGatewayProxyEvent): Object {
        if (!event) return {};
        ...
        if (!event.body && !event.queryStringParameters) { //lambda direct calls and step functions payload has no body and no queryStringParameters.
            ...
            else if (typeof event == "object") bodyString = JSON.stringify(event); //lambda direct call has object payload.
    ...
    private getObjectStringFromDictionary(dict): string {
        let obj = {};
        for (let key in dict) {
            let value = dict[key];
            obj[key] = value;
