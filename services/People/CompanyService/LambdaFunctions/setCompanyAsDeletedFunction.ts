...
export const setCompanyAsDeleted: APIGatewayProxyHandler = async (event, _context) => {
  ...
  if (!companyToUpdate.isPKDefined()) { //if not is PK defined
    return Utils.getUniqueInstance().getValidationErrorResponse(requestBody, companyToUpdate.getReadAndDeleteExpectedBody());
  }
